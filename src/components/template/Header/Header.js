...
export default withRouter(
  class Header extends Component {
          ...
          {navToUse.map((navLink, i) => {
            return (
              <React.Fragment key={i}>
                ...
                {i < navToUse.length - 1 ? (
                  <li
                    className={activeNav && parseInt(activeNav) > i ? 'step-bar past' : 'step-bar'}
                  >
                    <span></span>
                  </li>
                ) : null}
    ...
    getDefaultPageTitle = defaultText => {
      const isDefaultCompactViewPage = this.state.compactViewPages[
        this.props.pagePath.split('/')[1]
      ];
